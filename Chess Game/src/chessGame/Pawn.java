package chessGame;

public class Pawn extends Pieces{
	public Pawn(int color, int x, int y, Player player)
	{
		super(color, x, y, player);
	}
	//create another 2D array to store the valid move of Pawn
	//initialized all positions to 0, set to 1 if valid move
	public int[][] validMove(Chessboard Board) {
		int[][] validBoard = new int[8][8];
		for(int i = 0; i < 8; i++){
			for(int j = 0; j < 8; j++){	
				validBoard[i][j] = 0;
			}
		}
		
		int currx = this.getx();
		int curry = this.gety();
		int currcolor = this.getcolor();
		
		//if pawn is black, moves down
		if(currcolor == 0){
			int newx, newy;
			newx = currx;
			newy = curry-1;
			if(Board.getPosition(newx, newy) == null){
				validBoard[newx][newy] = 1;				
			}
			//pawn's first move
			if(curry == 6){
				newy = curry-2;
				if(Board.getPosition(newx, newy) == null){
					validBoard[newx][newy] = 1;
				}
			}
			
			//capture enemy pieces
			newx = currx+1;
			newy = curry-1;
			capture_enemy(Board, validBoard, currcolor, newx, newy);
			
			newx = currx-1;
			newy = curry-1;
			capture_enemy(Board, validBoard, currcolor, newx, newy);
			
		}
		//if pawn if white, moves up
		else{
			int newx, newy;
			newx = currx;
			newy = curry+1;
			if(Board.getPosition(newx, newy) == null){
					validBoard[newx][newy] = 1;
					
					
			}
			//pawn's first move
			if(curry == 1){
				newy = curry+2;
				if(Board.getPosition(newx, newy) == null)
				{
					validBoard[newx][newy] = 1;
				}
			}
			
			//capture enemy pieces
			newx = currx+1;
			newy = curry+1;
			capture_enemy(Board, validBoard, currcolor, newx, newy);
			
			newx = currx-1;
			newy = curry+1;
			
			capture_enemy(Board, validBoard, currcolor, newx, newy);
			
		}
			
		return validBoard;
	}


}